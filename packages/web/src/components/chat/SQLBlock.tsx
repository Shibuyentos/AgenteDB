...
export const SQLBlock = memo(function SQLBlock({ sql, executed = false, onResult }: SQLBlockProps) {
  const [copied, setCopied] = useState(false);
  const [executing, setExecuting] = useState(false);
  const [wasExecuted, setWasExecuted] = useState(executed);

  // Memoize the highlight output — only recompute when SQL changes
  ...
  return (
    <div className="rounded-[1.5rem] overflow-hidden glass-card-strong border-white/5 my-4 shadow-glass-lg relative group">
      {/* Glow Effect on Hover */}
      <div className="absolute inset-0 bg-gradient-brand opacity-0 group-hover:opacity-[0.03] transition-opacity duration-300 pointer-events-none" />
      ...
      <div className="flex items-center justify-between px-5 py-3 bg-white/[0.02] border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleCopy}
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-bold text-text-muted hover:text-text-primary hover:bg-white/5 transition-colors cursor-pointer border border-transparent hover:border-white/10"
          >
          ...
          {!wasExecuted && (
            <button
              ...
              disabled={executing}
              className="flex items-center gap-2 px-4 py-1.5 rounded-xl text-[10px] font-black bg-gradient-brand text-white hover:scale-105 transition-transform cursor-pointer disabled:opacity-30 glow-brand shadow-glow-sm"
            >
              {executing ? (
      ...
      <div className="relative">
        <pre className="px-6 py-5 bg-black/40 text-[14px] font-mono overflow-x-auto leading-relaxed custom-scrollbar">
          <code className="block min-w-max">{highlighted}</code>
        </pre>
      </div>
    </div>
  );
